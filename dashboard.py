...
price_max = int(df["price"].dropna().max()) if df["price"].notna().any() else 1000000
...
c1.metric("Total Listings", human_format(df_filtered["property_id"].nunique()))
c2.metric("Avg Price", f"${human_format(df_filtered['price'].mean() if df_filtered['price'].notna().any() else 0)}")
c3.metric("Median Price", f"${human_format(df_filtered['price'].median() if df_filtered['price'].notna().any() else 0)}")
c4.metric("Listings with coords", human_format(df_filtered.dropna(subset=["latitude", "longitude"]).shape[0]))
...
if not df_filtered.empty:
    ...
    if not samp_types.empty:
        fig_type = px.scatter_mapbox(
            ...
            hover_name="address", size="price", zoom=map_zoom, center=center,
            mapbox_style="open-street-map", color_discrete_sequence=COLOR_CATEG
        )
    ...
    if not samp_types.empty:
        fig_status = px.scatter_mapbox(
            samp_types, lat="latitude", lon="longitude", color="property_status",
            hover_name="address", size="price", zoom=map_zoom, center=center,
            mapbox_style="open-street-map", color_discrete_sequence=COLOR_CATEG
        )
    ...
    if "bedroom_number" in df_filtered.columns:
        ...
        if not beds_df.empty:
            ...
            fig_beds = px.scatter_mapbox(
                ...
                hover_name="address", mapbox_style="open-street-map", zoom=map_zoom, center=center,
    ...
    if "living_space" in df_filtered.columns:
        ...
        if not df_space.empty:
            ...
            fig_space = px.scatter_mapbox(
                ...
                hover_name="address", hover_data=["living_space", "price", "bedroom_number"],
